/**
 * Created by YCXJ-wanglihui on 2014/9/1.
 */
'use strict';
...
/**
 * 将查询记录分页显示
 * @param options[page,perPage,offset]
 * @param callback
 */
Query.prototype.paginate = function(options, callback){
    ...
    var page = options.page >0 ? options.page : defaultOptions.page;
    var perPage = options.perPage >0 ? options.perPage : defaultOptions.perPage;
    var offset = options.offset >= 0 ?options.offset : defaultOptions.offset;
    ...
    ep.all("total", "items", function(total, items){
        var pg = new Paginate(page, perPage, total, items);
        callback(null, pg);
    });
    query.skip(skip).limit(perPage).exec(ep.done("items"));
    model.count(query._conditions).exec(ep.done("total"));
}
